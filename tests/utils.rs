...
pub async fn spin_up_validator(port: Option<u16>) -> eyre::Result<(SocketAddr, BlsPublicKey)> {
    let dummy_sk = random_bls_secret();
    let pubkey = dummy_sk.sk_to_pk();
    let validator = Validator::new_in_memory(dummy_sk, port.unwrap_or(0)).await?;
